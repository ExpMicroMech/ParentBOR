function [ebsd, header] = loadEBSD_h5_light(fname,varargin)

% TPM modified 03/2020 - now skips over raw pattern hdf5 entries unless
% specified.

% Give the tag 'read_patterns' after fnmae in order to make the function
% try to read the raw patterns (which is probably quite an unusual
% situation, as it return an EBSD object which doesn't contain any
% patterns)
...
if numel(ginfo) > 0
    ...
    for k=1:numel(varargin)
        if ischar(varargin{k})
            ...
            %look for the string 'skip_patterns'
            read_raw_patterns = strncmpi(varargin{k},'read_patterns',numel(varargin{k}));
        end
    end
    ...
    %try
        for k = 1:numel(ginfo)
            ...
            for j=1:numel(kGroupData)
                ...
                %if you don't want to load the raw patterns then skip this
                %entry
                tag={kGroupData(j).Name};
                pattern_flag=strncmpi(tag,'RawPatterns',numel('RawPatterns'));
                
                %disp(tag{:})
                if pattern_flag==1&&read_raw_patterns==0
                    continue
                elseif pattern_flag==1
                    disp('Reading raw patterns')
                end

                data = double(h5read(fname,[kGroup.Groups(1).Name '/' tag{:}]));
