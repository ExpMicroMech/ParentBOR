function [PlotOpt]=Plotting_Part0(settings,PlotOpt,grains,ebsd_all)
...
if PlotOpt.IPFs.BCC.raw > 0;
    ...
    plotMaps_BCC_init(ebsd_all,grains,settings,PlotOpt,1); 
...
if PlotOpt.IPFs.BCC.processed > 0;
    ...
    plotMaps_BCC_init(ebsd_all,grains,settings,PlotOpt,2);
    %plotMaps_BCC(ebsd_all,grains,settings,PlotOpt,Type)
    close;
end
...
if PlotOpt.IPFs.BCC.smoothed > 0;
    ...
    plotMaps_BCC_init(ebsd_all,grains,settings,PlotOpt,3); 
    %plotMaps_BCC(ebsd_all,grains,settings,PlotOpt,Type)
    close;
end
